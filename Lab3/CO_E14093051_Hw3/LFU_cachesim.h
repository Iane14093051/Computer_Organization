...
class lfsr_t
{
 public:
  ...
  {
  	LFU_array = new uint32_t[sets*ways](); 
  ...
  uint32_t LFU(size_t ways, size_t idx)
  {
  	uint32_t index=0;
  	  if(num_array[idx]==ways)
  	  {
  	  	uint32_t min=LFU_array[idx*ways];
  	  	for(uint32_t i=1;i<ways;i++)
  	  	{
  	  		if(min>LFU_array[idx*ways+i])
  	  		{
  	  			index = i;
  	  			min = LFU_array[idx*ways+i];
			}
		}
		
		LFU_array[idx*ways+index]= 0;
	  }
	  else
	  {
	  	index = num_array[idx];
	  	LFU_array[ways*idx+index] = 0;
	  	num_array[idx]++;
	  }
	  return index;
  }
  uint64_t LFU_full(size_t ways,uint64_t locate)
  {
