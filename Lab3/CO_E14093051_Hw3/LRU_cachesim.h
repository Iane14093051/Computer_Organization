...
class lfsr_t
{
 public:
  ...
  uint64_t LRU_full(size_t ways,uint64_t locate)
  {
  	  	...
  	  	map<uint64_t, int>::iterator i=full_ma.begin();
  	  	i++;
  	  	uint64_t temp =0;
  	  	for(;i!=full_ma.end();i++)
  	  	{
    ...
    void insert(uint64_t line)
  {
	max_ren++;
	full_ma[line] = max_ren;
	/*full_ma.insert(std::pair<uint64_t, int>(line,max_ren));
	while(full_ma.find(line)==full_ma.end())
	{
		full_ma.insert(std::pair<uint64_t, int>(line,max_ren));
	}*/
  }
